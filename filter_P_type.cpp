...
bool has_only_P_type_endpoints(const Topology_enhanced& T) {
    try {
        ...
        std::vector<int> b0_final = tensor.Getb0Q();  // ✅ Get b_0Q after blowdown
        ...
        // SAFETY: Check b0_final size
        if (b0_final.size() != (size_t)(IF_final.rows() + 1)) {
            std::cerr << "[Warning] b0Q size mismatch: " 
                      << b0_final.size() << " vs " << (IF_final.rows() + 1) << "\n";
            return false;
        }
        ...
        if (n == 1) {
            // P-type requires: self-intersection = 0 AND b0 = 2
            return (IF_final(0, 0) == 0) && (b0_final[0] == 2);
        }
        ...
        // 6. Check all endpoints satisfy P-type conditions:
        //    (a) self-intersection = 0
        //    (b) b_0Q value = 2  ✅ NEW CONDITION
        for (int ep : endpoints) {
            ...
            if (ep >= (int)b0_final.size()) {
                std::cerr << "[Warning] b0 index out of bounds: " << ep << "\n";
                return false;
            }
            
            // Check condition (a): self-intersection = 0
            if (IF_final(ep, ep) != 0) {
                return false;
            }
            
            // Check condition (b): b_0Q = 2  ✅ NEW
            if (b0_final[ep] != 2) {
                return false;
            }
        }
